...
| RT-002 | P0 | 장중 schedule은 5분 간격이며 정각을 피한다. | cron이 `04`분부터 5분 간격이고 NY session gate를 통과한다. |
...
| MKT-012 | P0 | 같은 방향의 다음 구간 알림은 직전 가격 알림 이후 달라진 맥락을 먼저 표시한다. | +4에서 +5로 진행하면 판정·상대 흐름·거래량·신규 근거의 변화만 별도 블록에 표시하고, 변화가 없으면 새 구간 진입만 명시한다. |
| MKT-013 | P0 | 설명되지 않은 MOVE 뒤 신뢰 가능한 근거가 확인되면 원래 가격 사건에 후속 확인 한 건을 연결한다. | MOVE 전후 6시간의 공식/주요 원보도는 부모 event key를 가진 후속 알림 한 건만 만들고 인과관계 미확정을 표시한다. |
| MKT-014 | P0 | 지연·복구 MOVE는 과거 관측과 마지막 관측 상태를 분리한다. | 제목을 `도달 기록`으로 바꾸고 10분 초과 시 `지연 확인`을 추가한다. 관측·봇 확인 시각을 KST 날짜와 함께 본문/fallback에 표시한다. 마지막 관측의 세션·시각·방향·구간 유지/되돌림/반등과 그 시점 상대 성과를 표시한다. |
| MKT-015 | P0 | 뒤늦은 거래량을 과거 가격 사건에 동반된 거래량처럼 표현하지 않는다. | 실제 포함한 마지막 정규장 봉 시각을 별도 보존한다. 애프터마켓·조기폐장·누락된 마지막 봉·재시작·v10 이전 DB의 미기록 시각을 검증한다. 복구 MOVE의 도달 당시 변화 비교에서는 거래량 변화를 제외한다. |
| MKT-016 | P0 | 오래된 마지막 관측을 현재 상태로 단정하지 않는다. | 마지막 관측도 확인 시각보다 10분 초과 오래됐으면 이후 상태 미확인을 표시한다. 정확한 가격/관측 수익률은 계속 비노출하며 새 상태 설명 때문에 알림을 추가 발송하지 않는다. |
...
| REL-005 | P0 | 상대 성과는 beta 보정 전 실제 수익률 차이로 분류한다. 절대 차이 0.3%p 이하는 비슷한 흐름, 0.3 초과~0.8%p 이하는 약간, 0.8 초과~2.0%p 이하는 상당한, 2.0%p 초과는 강한 아웃퍼폼·언더퍼폼이다. | 양·음 방향의 0.3, 0.8, 2.0 경계와 바로 위·아래 값을 검증하며 SOXX·피어 및 MOVE·VOLUME·CLOSE·WEEKLY에 동일하게 적용한다. |
| REL-006 | P0 | 상대 계산 수치는 내부에만 저장한다. | 사용자에게 정확한 종목/지수/피어 수익률이 노출되지 않는다. |
| REL-007 | P0 | 최근 6개월 일봉으로 종목의 SOXX 및 동일가중 피어 바스켓 민감도와 잔차 범위를 계산한다. | 공통 날짜 40개 이상일 때 beta와 잔차 밴드가 생성되고 profile별로 저장된다. |
| REL-008 | P0 | 민감도 모델은 7일마다 갱신하고 일시 실패 시 30일 이내 캐시만 사용한다. | Yahoo 갱신 실패 fixture에서 30일 이내 모델은 유지되고 초과 모델은 폐기된다. |
| REL-009 | P0 | 민감도 기준 기대 움직임 비교는 실제 상대 성과와 분리된 보조 판정이다. | 실제 차이가 1.4%p이고 beta 보정 기대 범위 안이어도 `상당한 아웃퍼폼`을 유지하며 모델 판정은 그 뒤 `민감도 참고`로 표시한다. |
| REL-010 | P0 | SOXX와 피어가 모두 기대 범위를 같은 방향으로 벗어날 때만 종목 고유 강·약세 가능성을 높게 표시한다. | 한 비교축만 이탈하면 혼합으로 표시하되, 시장·피어가 상승·하락의 원인이라고 단정하지 않는다. |
| REL-011 | P0 | 데이터나 모델이 부족하면 종목 고유 원인을 단정하지 않는다. | 비교축이 없으면 `판단 보류`, 모델이 없으면 낮은 신뢰의 고정 밴드 판정으로 폴백한다. |
| REL-012 | P0 | 후속 MOVE는 같은 방향의 상대 강도 변화도 표시하되 신규 알림 트리거를 만들지 않는다. | 다음 가격 구간에서 약간→상당한→강한 변화를 표시한다. 기존 beta 보정 context에서 새 실제 차이 context로 전환한 것을 시장 변화로 표시하지 않는다. |
...
| CAT-009 | P0 | MOVE에는 발생 전후 6시간의 공식 또는 주요 원보도 촉매 최대 2건만 넣는다. | 오래된 기사와 2차 분석은 제외되고, 적격 근거가 세 건 이상이어도 상위 두 건만 표시된다. |
...
| CAT-016 | P0 | 2차 출처는 고중요도 사건이어도 독립 즉시 알림을 만들지 않는다. | secondary acquisition 후보는 briefing에 남고 outbox는 생성되지 않는다. |
| CAT-017 | P0 | 같은 사건의 공식 원문 또는 주요 원보도가 뒤늦게 확인되면 사건을 승격할 수 있다. | secondary 후보 뒤 IR/Reuters 후보가 도착하면 canonical cluster는 유지하면서 신뢰 출처 알림 한 건만 생성된다. |
| CAT-018 | P0 | 가격 움직임과 근거의 시간상 인접성을 인과관계로 표현하지 않는다. | 후속 확인 메시지는 발생·게시 시각과 방향 정합성을 보여주되 `원인`으로 단정하지 않는다. |
...
| DLV-009 | P0 | 가격 사건의 판정 맥락과 근거 연결 상태를 영속화한다. | runner 재시작 뒤에도 다음 가격 구간의 delta와 MOVE 후속 근거가 중복 없이 이어진다. |
...
2026-09-18 시점 표시 개정: `지연 감지`의 사용자 표기는 `지연 확인`으로 통일한다.
정확한 최초 도달 시각을 알 수 없으므로 공급자 timestamp는 `발생` 대신 `관측`으로
표시한다. 마지막 관측과 봇 확인 시각은 서로 다를 수 있다. 복구 메시지의 상대 성과와
민감도 참고는 마지막 관측 기준으로 계산하고, 도달 당시 변화 비교는 별도로 표시한다.
CLOSE도 미국 거래일·시장 자료 시각·브리프 작성 시각을 명시해 장후 MOVE와 혼동하지
않게 한다. Slack의 수신 시각과 봇 확인/작성 시각을 동일한 값으로 취급하지 않는다.
