...
| REL-005 | P0 | 유효한 민감도 모델이 없을 때만 ±0.5%p 이내 차이를 동조로 분류한다. | 모델 부재 fixture가 아웃퍼폼/동조/언더퍼폼을 기존 경계로 정확히 나눈다. |
...
| REL-009 | P0 | 당일 실제 움직임을 민감도 기준 기대 움직임과 비교한다. | 단순 수익률 차이로는 아웃퍼폼이어도 고 beta 종목의 기대 범위 안이면 동조로 판정한다. |
| REL-010 | P0 | SOXX와 피어가 모두 기대 범위를 같은 방향으로 벗어날 때만 종목 고유 강·약세 가능성을 높게 표시한다. | 한 비교축만 이탈하면 `혼합`, 둘 다 설명 범위면 `시장·피어 흐름으로 설명`으로 표시한다. |
