...
1. 임계치 사건
2. 최근 6개월 민감도를 반영한 종목 고유/시장 설명/혼합 판정
3. SOXX 대비 결과
4. ETN/GEV/NVT 동일가중 대비 결과
5. 동시간대 거래량 강도
6. 발생 전후 6시간 내 공식 또는 주요 원보도 촉매 최대 2건
7. 출처

같은 방향의 다음 정수 구간에서는 직전 가격 알림 이후 달라진 판정, 상대 흐름,
거래량, 신규 근거만 먼저 표시한다. 설명되지 않은 가격 사건 뒤 6시간 안에 신뢰
가능한 근거가 새로 확인되면 가격 사건과 연결된 후속 확인을 한 번만 보낸다. 이때
시간상 인접성과 방향 정합성은 표시하되 인과관계로 단정하지 않는다.
...
- 2차 요약 매체만으로는 독립 즉시 알림을 만들지 않는다. 같은 사건의 회사 원문이나
  주요 원보도가 확인되면 기존 canonical 사건을 승격해 한 번만 알린다.
