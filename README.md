...
- Stage 2 시장 경로는 XNYS 휴장·조기폐장 캘린더, Yahoo 5분봉, 인증 multi-quote, 고정 전일 종가, SOXX/피어 상대 흐름, 동시간대 20세션 거래량과 intraday replay를 구현했습니다. 상대 성과는 종목 수익률에서 SOXX 또는 유효 피어의 동일가중 평균 수익률을 뺀 실제 차이로 표시합니다. 절대 차이 `0.3%p 이하`는 비슷한 흐름, `0.3 초과~0.8%p 이하`는 약간, `0.8 초과~2.0%p 이하`는 상당한, `2.0%p 초과`는 강한 아웃퍼폼·언더퍼폼입니다. MOVE·VOLUME·CLOSE·WEEKLY가 같은 경계를 사용하며 실제 수익률과 차이 수치는 노출하지 않습니다. 최근 6개월 일봉 기반 민감도 모델은 `민감도 참고`로 분리해 상대 성과를 덮지 않습니다. 모델 갱신 실패 시 30일 이내 캐시만 사용하고 그 이후에는 모델 없는 상대 흐름 참고로 폴백합니다. +5, +6처럼 후속 가격 구간에서는 직전 알림 뒤 바뀐 상대 흐름·강도·거래량·근거만 먼저 보여줍니다. 새 거래일의 첫 GitHub 실행이 늦어져도 당일 04:00 ET 이후 봉을 처음부터 복원해 오전 급변 후 되돌림을 놓치지 않습니다. 복구된 MOVE는 `도달 기록`, 10분 초과 지연은 `지연 확인`으로 제목부터 구분하고 관측·봇 확인·마지막 관측·거래량 시각을 분리합니다. 아래 지연 알림 기준을 따릅니다.
- Stage 3 근거 경로는 Yahoo Search와 발행사 IR RSS, 결정론적 저가치 필터, 사건 클러스터링, 원문 근거 검증 AI 분석, SEC 403 Yahoo 미러 복구, 신규 공시 기준선과 재시도 ledger를 구현했습니다. 뉴스 제목에 회사를 직접 명시하지 않은 업종 일반론과 목표가·주가 해설·펀드 보유 기사는 AI 전에 제외합니다. 검증된 사실과 거래 금액까지 비교해 7일 안의 재전재를 최초 canonical 사건에 연결하고, 최근 촉매는 사건당 한 건만 노출합니다. 같은 AI 응답에서 사건 유형·회사 직접성·새 사실·중요도·출처 계층을 구조화하고, 고신뢰·고중요도의 인수·실적/가이던스·대형 계약/고객·경영진·규제 사건만 독립 알림으로 허용합니다. 이 경우에도 2차 출처만으로는 즉시 알림을 만들지 않으며 회사 IR·SEC 또는 주요 원보도가 확인돼야 승격합니다. 설명되지 않은 가격 알림 전후 6시간 안에 신뢰 가능한 근거가 뒤늦게 도착하면 별도 사건 알림 대신 가격 사건에 연결된 후속 확인을 한 번만 보냅니다. 생산능력·제품·파트너십·자금조달은 브리핑 전용, 불확실하거나 논평성인 자료는 ledger 전용입니다.
...
- `Monitor V2 Production`의 `preview` 모드는 MOVE 상승·하락, 거래량, 회사 사건, 가격 움직임 후속 근거, SEC 공시, 장 마감, 주간 리뷰 fixture를 production과 같은 Slack adapter로 전송합니다. 모두 `미리보기`와 `실제 투자 신호가 아님`을 명시하며 지정한 preview event만 전송해 기존 outbox를 소비하지 않습니다.
...
- `관측`은 공급자가 기록한 시각이며 정확한 최초 임계치 통과 시각을 뜻하지 않습니다. `봇 확인`은 해당 데이터를 처리한 시각입니다. 날짜와 KST를 본문 및 Slack fallback(휴대폰 알림)에 함께 넣습니다.
- 복구 MOVE는 `+4.0% 상승 도달 기록`처럼 과거 기록임을 명시하고, 관측에서 확인까지 10분을 초과하면 제목에도 `지연 확인`을 붙입니다.
- 마지막 관측의 세션·시각, 양전/음전/보합, 해당 구간 유지/되돌림/반등을 표시합니다. 상대 성과와 민감도 참고는 그 마지막 관측 기준입니다. 마지막 자료마저 확인 시각보다 10분 초과 오래됐으면 이후 상태는 미확인임을 밝힙니다. 정확한 현재 등락률과 가격은 노출하지 않습니다.
- 거래량은 별도 정규장 누적 자료입니다. schema v11의 `data_observed_at`에 마지막으로 포함한 정규장 봉의 시각을 저장하고 재시작 뒤에도 유지합니다. 애프터마켓 시각을 정규장 거래량 시각으로 사용하지 않고, 예전 DB의 미기록 시각도 추정하지 않습니다. 과거 도달 기록의 변화 요약에 나중 거래량을 끼워 넣지 않습니다.
- CLOSE에는 미국 거래일, 시장 관측 시각, 브리프 작성 시각을 표시합니다. 정규장 데이터와 뒤늦은 전달 시각을 구분합니다. Slack에 표시되는 메시지 시각이 실제 수신 시각이며 `봇 확인`/`작성`을 전송 완료 시각으로 부르지 않습니다.
- 시간 표시와 상태 설명만 바뀝니다. watermark, 이벤트 키, 재발송 규칙, Slack 전송 방식과 cron은 유지합니다. 새 유료 서비스나 추가 API 호출은 없습니다.
