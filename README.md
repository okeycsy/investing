...
- 품질 리포트의 Stage 5 판정은 GitHub `schedule` poll coverage와 Yahoo 5분봉 복구 coverage를 분리합니다. 스케줄 지연은 감추지 않고 advisory로 남기되, XNYS 조기폐장을 포함한 세션별 5분 구간을 모두 복원했는지로 전체 거래일을 계산합니다. 거래일당 비정기 알림 0~3건, 최근 2개 완료 거래일의 저가치·중복 근거 0건도 release gate로 사용합니다. 조건을 채우기 전에는 `blocked` 또는 `observing`이며 production 전환을 허용하지 않습니다.
